        ...
        <div className="p-4 flex flex-col gap-3">
          ...
          {isImage && (
            <div className="rounded-lg border border-gray-200 overflow-hidden bg-gray-50">
              <img src={result.url} alt={t('export.previewAlt')} className="w-full h-40 object-contain" />
            </div>
          )}

          <div className="text-xs text-gray-500 truncate">
            {t('export.filename')} <span className="font-mono text-gray-700">{result.filename}</span>
          </div>
          ...
          >
            <ExternalLink size={16} /> {t('export.openNewTab')}
          </a>
        </div>
      </div>
    </div>
