...
export function ScoreCurve({ phases, project }: { phases: Phase[]; project: ProjectSettings }) {
  if (phases.length === 0) return null
  const bands = computeRowBands(project)
  const band = bands.dataBands[3] // score row
  const localTop = band.top - bands.rowsAreaTop
  const top = localTop + 8
  const bottom = localTop + band.height * 0.55

  const points = phases.map((p, i) => {
    const frac = Math.max(0, Math.min(1, (p.score - 1) / 9)) // score 1–10 -> 0–1
